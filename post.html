...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .category { display:inline-block; font-size:11px; color:#8ab4f8; background:#1a3a6b; padding:3px 10px; border-radius:10px; margin-bottom:10px; }
  h1 { font-size:22px; font-weight:700; margin-bottom:8px; }
  .meta { font-size:12px; color:#9aa0a6; margin-bottom:18px; }
...
<script>
  ...
  async function loadPost() {
    ...
    if (!slug) { el.innerHTML = '<div class="error">Post not specified.</div>'; return; }
    try {
      const res = await fetch("/api/posts");
      const data = await res.json();
      const post = (data.posts || []).find((p) => p.slug === slug);
      if (!post) { el.innerHTML = '<div class="error">Post not found.</div>'; return; }

      document.title = post.title + " — MHBuilder Lahore";
      document.getElementById("pageTitle").textContent = post.title + " — MHBuilder Lahore";
      document.getElementById("pageMeta").setAttribute("content", post.meta_description || "");
      ...
      let html = "";
      if (post.image_url) html += `<img class="hero" src="${post.image_url}" alt="${post.title}">`;
      if (post.category) html += `<div class="category">${post.category}</div>`;
      html += `<h1>${post.title}</h1>`;
      html += `<div class="meta">📅 ${dateStr}</div>`;
      html += `<div class="content">${post.content || ""}</div>`;
      html += shareRowHtml(post.title, window.location.href);
      el.innerHTML = html;
    } catch (e) {
      el.innerHTML = '<div class="error">Could not load this post.</div>';
    }
  }
