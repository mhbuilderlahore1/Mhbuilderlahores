...
export async function onRequestGet({ env }) {
  ...
  const withUrls = results.map((p) => ({ ...p, logo_url: p.r2_key ? `${PUBLIC_SITE}/photos/${p.r2_key}` : null }));
...
export async function onRequestPost({ request, env }) {
  ...
  const result = await env.DB.prepare(`INSERT INTO partners (name, r2_key, link, uploaded_by) VALUES (?, ?, ?, ?)`)
    .bind(name, r2Key, link, env.ADMIN_USER)
    .run();
